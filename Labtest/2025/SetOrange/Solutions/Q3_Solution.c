...
int calculateSectionMax(struct Student section[]) {
    int max = section[0].math_marks;
    for(int i = 1; i < STUDENTS_PER_SECTION; i++){
        if(section[i].math_marks > max)
            max = section[i].math_marks;
    }
...
void sortPointerArray(struct Student* SectionArr[]) {
    for(int i = 0; i < NUM_SECTIONS; i++){
        ...
        for(int j = i + 1; j < NUM_SECTIONS; j++){
            if(calculateSectionMax(SectionArr[j]) > calculateSectionMax(SectionArr[max_index]))
                max_index = j;
        }

        struct Student* temp_section = SectionArr[i];
        SectionArr[i] = SectionArr[max_index];
        SectionArr[max_index] = temp_section;
    }
