...
void sortPointerArray(Student *SectionArr[], int n) {
    for(int i = 0; i < n; i++){
        int min_index = i;
        for(int j = i + 1; j < n; j++){
            if(calculateSectionAverage(SectionArr[j]) < calculateSectionAverage(SectionArr[min_index]))
                min_index = j;
        }

        Student* temp_section = SectionArr[min_index];
        SectionArr[min_index] = SectionArr[i];
        SectionArr[i] = temp_section;
    }
